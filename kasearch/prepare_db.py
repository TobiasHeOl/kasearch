...
class PrepareDB:
    ...
    def __init__(self, db_path, n_jobs=1, oas_source=False):
        ...
        self.tmpDB = tmpDB(collections.defaultdict(dict), collections.defaultdict(dict), collections.defaultdict(dict), collections.defaultdict(dict), collections.defaultdict(dict))
    ...
    def prepare_sequences(self, data_unit_file, file_id, sequence_lines = None, chain='Heavy', species='Human'):
        """
        ...
        sequence_alignments = AlignSequences(sequences, n_jobs=self.n_jobs).db.aligned_seqs
